  ...
  <Submenu :name="`${parentName}`">
    <template slot="title">
      ...
      <span>{{ showTitle(parentItem) }}
        <Badge v-if="parentItem.name=='order'" :count="0" class="order-badge-count"></Badge>
      </span>
    </template>
    <template v-for="item in children">
      ...
      <template v-else>
        ...
        <menu-item v-else :name="getNameOrHref(item)" :key="`menu-${item.name}`">
          <common-icon :type="item.icon || ''"/><span>{{ showTitle(item) }}
            <Badge v-if="item.name=='evaluated'" :count="0" class="order-badge-count"></Badge>
          </span></menu-item>
      </template>
    </template>
  </Submenu>
