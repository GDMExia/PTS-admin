<template>
  <div>
    <Form @submit.native.prevent="()=>{}" ref="SetForm" :model="formInline" :rules="ruleInline" :label-width="80">
      ...
      <Row>
        <Col span="20">
          <FormItem label="项目" prop="amusementItemList">
            <Select v-model="formInline.amusementItemList" multiple>
              <Option v-for="(item,inedx) of projectList" :value="item.id">{{item.projectName}}</Option>
            </Select>
          </FormItem>
        </Col>
      </Row>
...
export default {
  ...
  methods: {
    ...
    getalreadysetdate(){
      getitemdate(this.$store.state.user.token).then(res=>{
        console.log(res)
